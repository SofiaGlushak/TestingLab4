...
def test_dynamodb_table_exists(dynamo_resource):
    client = boto3.client("dynamodb", endpoint_url=AWS_ENDPOINT_URL, region_name=AWS_REGION)
    response = client.list_tables()
    assert "ShippingTable" in response["TableNames"]

# 10. Тест створення черги у SQS
def test_sqs_queue_exists():
    client = boto3.client("sqs", endpoint_url=AWS_ENDPOINT_URL, region_name=AWS_REGION)
